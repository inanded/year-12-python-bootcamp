...
function ChallengePlayer({ badge, level, levelIndex, progress, certifications, testProgress, onBack, onShowBadges, onSelectLevel, onToggleTask, onToggleTest, onSetCertification, onOpenBadge, onDownload, onUpload }: {
  ...
  const finalBuildStep = activeTaskIndex === level.tasks.length - 1;
          ...
          <div className="player-workspace">
            <article className="player-main" id="player-stage-panel" tabIndex={-1}>
              ...
              {activeStage === 2 && <section className="stage-panel">
                ...
                {!finalBuildStep && <details className="starter-box staged-reveal"><summary>Step {activeTaskIndex + 1} support · reveal an incomplete Python scaffold</summary><div className="starter-warning">Try the current step first. This support is deliberately incomplete and is tailored to the challenge: it may be a cloze, broken code or a structure with gaps. Use only the part needed for this step.</div><CodeBlock label={`Incomplete support for Step ${activeTaskIndex + 1}`} value={level.pythonStarter} /></details>}
                {finalBuildStep && <details className="model-box build-model staged-reveal"><summary>Step 3 only · reveal the full model Python answer</summary><div className="model-warning">This is the complete solution. Open it only after making and running your own Step 3 attempt. Compare one section at a time, then return to your own file rather than replacing it.</div><CodeBlock label="Full model Python" value={level.model} /><h4>Why this works</h4><ul>{level.modelNotes.map((note) => <li key={note}>{note}</li>)}</ul></details>}
